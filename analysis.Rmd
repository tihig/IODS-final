...
Overall view of the prediction looks successful. 

```{r curve, echo=FALSE}
m3 <- glm(high_points ~ interest, data = learn, family = "binomial")
plot(learn$interest,learn$high_points,xlab="Interest",ylab="Probability of high points")
curve(predict(m3, data.frame(interest = x), type = "response"),add=TRUE) 
points(learn$interest,fitted(m3),pch=20)
```



Incase nothing else is found, use this one on the below: 


```{r ggplots, echo=FALSE}
g <- ggplot(learn, aes(x = probability, y = high_points, col= prediction))

g + geom_point()

table(high_points = learn$high_points, prediction = learn$prediction) %>% prop.table() %>% addmargins()
